...
| `semantic-only` | The site reaches past the semantic layer into primitives. Often hidden behind a friendly local alias (`--cream: var(--…-primitives-neutral-100)`), which looks like good CSS and reads like a token | The site — swap to the semantic token the report names. If none carries that value, the semantic layer has a gap: add it in Figma rather than reaching around it |
